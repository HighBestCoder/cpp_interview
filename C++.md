...
面试里可以直接回答：封装就是把数据和操作数据的方法放到类里，对外提供接口，对内隐藏实现细节。

它的核心价值是降低耦合、保护数据、提高代码可维护性。

**继承**

继承就是子类复用父类已有的成员和接口，并在这个基础上做扩展。

它的核心作用是代码复用和建立类之间的层次关系。工程里最常用的是单继承，多继承要注意菱形继承问题，通常靠虚继承解决。

**多态**

多态可以概括为同一套接口，不同对象表现不一样。

C++里常见两类多态：编译期多态，比如函数重载、模板；运行期多态，比如父类指针或引用调用虚函数时，实际执行的是子类重写后的函数。

一句话总结就是：封装是隐藏实现，继承是复用扩展，多态是统一接口、不同实现。

**面试补充：容易追问的点**

1. 虚继承：主要是为了解决菱形继承中的公共基类重复问题，避免子类里有多份父类数据。
2. 虚析构：如果一个类会被当成父类来用，也就是会通过父类指针删除子类对象，那么父类析构函数必须是虚函数，否则可能析构不完整。
3. 构造函数不能是虚函数：因为对象构造时虚函数表还没完全稳定，没法靠虚函数机制完成构造。
4. 多态生效条件：一是父类里有虚函数，二是通过父类指针或引用调用；如果是对象直接赋值，会发生对象切片，多态失效。
5. override最好显式写：这样可以避免因为函数签名不一致，结果没有真正重写成功。
6. 构造和析构阶段不要依赖虚函数：这两个阶段调用虚函数时，不会按“完整子类对象”那套多态逻辑工作，面试里这是常见追问。
7. public继承表达 is-a 关系：如果只是复用功能，不一定适合继承，很多时候组合比继承更稳。
...
面试里可以直接回答：面向过程是按步骤拆解问题，关注“怎么做”；面向对象是按对象和职责拆解问题，关注“谁来做”。

**面向过程**

优点：简单直接，执行路径清晰，运行时开销通常更小，适合性能敏感、流程固定的场景。

缺点：代码容易分散，复用、扩展、维护性一般，系统一复杂就容易失控。

**面向对象**

优点：职责划分清晰，易维护、易复用、易扩展，适合复杂业务建模。

缺点：设计成本更高，如果抽象过度，反而会让系统变复杂，也会带来一定性能开销。

**工作里怎么取舍**

1. 如果是底层模块、性能敏感模块、简单流程，优先面向过程。
2. 如果是业务系统、需求变化多、需要长期维护，优先面向对象。
3. 实际开发里一般是混用，整体设计偏面向对象，局部性能热点或简单逻辑用面向过程实现。
...
3. char *a
  ...
  但是要注意对于char s[]和char* a，我们可以有`a = s`，但不能有`s = a`。

  原因不是a是不是空指针，而是s是数组名，数组名不是普通指针变量，不能被重新赋值；a是指针变量，本身存的就是地址，所以可以让它指向s。
  如果从更底层的角度理解，在编译器/汇编视角里，数组名更像一个固定地址，很多场景下会退化成首元素地址来使用，但它本身不是一个可写变量，所以不能写`s = a`。
...
- **static**
  ...
  - static对于类
    ...
    2. 成员函数
      ...
      原因是const成员函数本质上约束的是隐式的this指针，表示不能通过this修改对象状态；但static成员函数不属于某个具体对象，没有this指针，因此const在这里没有语义，C++就直接禁止了这种写法。

      也可以这样记：static成员函数更像“类作用域下的普通函数”，而const成员函数一定是“跟对象绑定的函数”，两者作用对象不一样，所以不能同时出现。
